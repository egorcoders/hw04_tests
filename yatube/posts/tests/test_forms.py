...
class PostsFormsTest(TestCase):
    ...
    def test_posts_forms_create_post(self):
        ...
        form_data = {'text': 'Тестовый пост формы', 'group': self.group.id}
        self.authorized_client.post(reverse('posts:post_create'),
                                    data=form_data)
        self.assertEqual(Post.objects.count(), post_count + 1)
        self.assertTrue(Post.objects.filter(text='Тестовый пост формы',
                                            group=self.group.id).exists())

    def test_posts_forms_edit_post(self):
        """Проверка, редактируется ли пост."""
        form_data = {'text': 'Новый текст поста', 'group': self.group.id}
        ...
        self.assertTrue(Post.objects.filter(text='Новый текст поста',
                                            group=self.group.id).exists())
